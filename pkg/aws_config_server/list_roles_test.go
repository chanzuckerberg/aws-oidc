...
func TestMapClientIDRoleARN(t *testing.T) {
	ctx := context.Background()
	r := require.New(t)
	ctrl := gomock.NewController(t)
	...
	policyData, err := json.Marshal(samplePolicyDocument)
	r.NoError(err)
	policyStr := url.PathEscape(string(policyData))
	...
	clientRoleMap := make(map[string][]ConfigProfile)
	err = MapClientIDRoleARN(ctx, "account3", oidcProvider, mock, clientRoleMap)
	r.NoError(err)
	// account3's testRoles1 role got added to clientRoleMap
	r.NotEmpty(clientRoleMap)
	r.Len(clientRoleMap, 1)
	firstConfigProfile := clientRoleMap["clientIDValue1"][0]
	r.Equal(firstConfigProfile.acctName, "account3")
}
