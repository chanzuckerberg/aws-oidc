# Tailscale OIDC enrollment for agent pods

Agent pods join the tailnet by exchanging a Kubernetes projected service-account
...
module "tailscale-federated-identity" {
  ...
  description = "Agent pods on dev-central EKS — OIDC token exchange for tag:mantis-shrimp"
...
|---|---|
| `issuer_url` | `https://oidc.eks.us-west-2.amazonaws.com/id/A72DBBC7C19B4B419EEA3497A3A4CC03` |
| `subject` | `system:serviceaccount:aws-oidc:*` (all agent service accounts in the operator namespace) |
| `scopes` | `["openid"]` |
| `tags` | `["tag:mantis-shrimp"]` |
...
1. The operator projects a service-account token with audience
   `api.tailscale.com/<client-id>` into each agent pod at
...
2. `docker/agent/entrypoint.sh` decodes the `aud` claim from the token's JWT payload to
   ...
   tailscale up --client-id=<client-id> --id-token=<token> \
     --advertise-tags=tag:mantis-shrimp --hostname=agent-<owner>-<name>
   ```
