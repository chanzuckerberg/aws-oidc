...
func init() {
	...
	operatorCmd.Flags().String(flagAgentImage, "", "Container image an agent runs when the agent does not name one")
	operatorCmd.Flags().StringSlice(flagAgentCommand, []string{"sleep", "infinity"}, "Command an agent runs when neither the agent nor its image provides a long-running entrypoint")
	operatorCmd.Flags().String(flagAgentStorageClass, "efs-agent-workspaces", "Storage class for the per-agent PVC; must be a ReadWriteMany class backed by the EFS CSI driver")
	operatorCmd.Flags().String(flagAnthropicFederationRuleID, os.Getenv("ANTHROPIC_FEDERATION_RULE_ID"), "Anthropic WIF federation rule ID (fdrl_...); when set with the other three anthropic flags, every agent pod receives a Claude WIF token and the four ANTHROPIC_* env vars")
	...
	operatorCmd.Flags().String(flagGitHubAppID, os.Getenv("GITHUB_APP_ID"), "Numeric app ID of the shared GitHub App agents clone and open pull requests as; when set with the installation id and the GITHUB_APP_PRIVATE_KEY environment variable, every agent pod receives the app's credentials")
	...
	operatorCmd.Flags().String(flagTailscaleTokenAudience, os.Getenv("TAILSCALE_TOKEN_AUDIENCE"), "Tailscale OIDC audience (api.tailscale.com/<client-id>); when set, Tailscale-enabled agents receive a projected token with this audience")
	operatorCmd.Flags().String(flagTailscaleTag, os.Getenv("TAILSCALE_TAG"), "Tailscale advertise tag (e.g. tag:mantis-shrimp); injected as TAILSCALE_TAG into agent pods")
	operatorCmd.Flags().String(flagManagedSettingsConfigMap, os.Getenv("MANAGED_SETTINGS_CONFIGMAP"), "Name of the ConfigMap holding managed-settings.json and ssh-guard.sh; when set, mounted at /etc/claude-code in every agent pod")
}
...
func operatorRun(cmd *cobra.Command, args []string) error {
	...
	mgr, err := ctrl.NewManager(restConfig, ctrl.Options{
		...
		Cache:                   cache.Options{ByObject: runtimeObjectCache(namespace)},
	})
	if err != nil {
	...
	// Agents only run where the cluster's OIDC issuer is configured, since without it their
	// pods have no way to assume their roles.
	var agentRuntime controller.RuntimeReconciler
	if clusterOIDCProvider != "" {
		agentRuntime = agentpod.New(mgr.GetClient(), mgr.GetScheme(), agentpod.Config{
			Namespace:                 namespace,
...
// runtimeObjectCache confines the caches for the objects an agent runtime uses to the
...
func runtimeObjectCache(namespace string) map[client.Object]cache.ByObject {
